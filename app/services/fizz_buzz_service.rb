class FizzBuzzService
  def initialize initial_page = 0, page_size = 100
    @range = 1..10**12 # 1 .. 100,000,000,000
  ...
  def change_page_size page_size, page = nil
    page ||= @initial_page
    page = page.to_i
    value = (serve page)[0][:value] #First Value with old page size
    
    @page_size = (page_size.to_i > 0)? page_size.to_i : 100
    
    @initial_page = find_page_by_value value
  end
